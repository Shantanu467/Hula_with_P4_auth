...
def install_hula_logic(mn_topo, switches, p4info_helper):
    for sw in mn_topo.switches():
        ...
        rtt_times_1.append(timer.time_elapsed)
        ...
        timer.record_end_timestamp()
        rtt_times_2.append(timer.time_elapsed)
        print(f'Switch : {sw} :: time : {timer.time_elapsed}')
...
def main(p4info_file_path, bmv2_file_path, topo_file_path):
    ...
    try:
        ...
        if 1:
            '''keys = [0,11111, 22222]'''
            #keys = [0, 13, 14, 15, 23, 24, 25, 113, 114, 115, 123, 124, 125]
            keys = [0, 111, 222, 333, 444, 555]
            # install local keys (needs to be changed later)
            register_id = p4info_helper.get_registers_id("MyIngress.keys")

            for i in range(1, len(keys)):
                packet = struct.pack('!iii', register_id, 0, keys[i])
                pkt_dgst = cksum([(1,2),(1,2),(register_id,8),(0,8),(keys[i],8)])
                packet_out = p4info_helper.buildPacketOut(
                    payload = packet,
                    metadata = {
                        1: b'\x00\x01',
                ...
                switches[f"s{i}"].PacketOut(packet_out)

            # read local key from the registers
            for sw in mn_topo.switches():
                if sw == 's6':
                    continue
                register_id = p4info_helper.get_registers_id("MyIngress.keys")
                packet = struct.pack('!iii', register_id, 0, 0)
                pkt_dgst = cksum([(1,2),(0,2),(register_id,8),(0,8),(0,8)])
                packet_out = p4info_helper.buildPacketOut(
                    payload = packet,
                    metadata = {
                        1: b'\x00\x01',
                ...
                print("Packet Out Sw : ", sw)
                switches[sw].PacketOut(packet_out)

            # insert the keys of neighbouring switches into each switch
            data = None
            with open("keys.json", "r") as out:
                data = out.read()
            port_map = json.loads(data)
        print("Port Map of Switches : ", port_map)
        insert_keys(mn_topo, switches, p4info_helper,port_map, keys)
...
#    print(f'\nRTT times : {rtt_times}')
    print(f'Average RTT time : {statistics.fmean(rtt_times_1) + statistics.fmean(rtt_times_2)}')
