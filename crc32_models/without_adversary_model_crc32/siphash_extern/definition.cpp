...
void siphash_digest(bm::Data& data, bm::Data &result){
	int c, d, m_idx;
        ...
        uint64_t pszChar_ = data.get<std::uint64_t>();
        uint64_t* pszChar = &pszChar_;

	std::cout<<"Got pszchar uint64_t : "<<pszChar_<<std::endl;
	while (pszChar != NULL && *pszChar != '\0') {
        ...
        uint64_t temp = ((uint64_t) v0 ^ v1 ^ v2 ^ v3);
	result = bm::Data{temp};
	std::cout<<"SipHash digest : "<<temp<<std::endl;
}


BM_REGISTER_EXTERN_FUNCTION(siphash_digest, bm::Data&, bm::Data&);
